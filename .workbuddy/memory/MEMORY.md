...
## 红点匹配 → 联系人名：OCR 垃圾名坑（核心教训）
- **`公众号` 不能放进 `_is_valid_contact_name` 黑名单**：公众号订阅号行的真名就叫"公众号"，黑掉它会让匹配器拿不到名字、回退到下方预览文字 `[18条]南阳本地宝：` → 存成垃圾联系人。导航栏"公众号"图标由 `x<nav_end` 位置过滤已挡住。
- **OCR 常把 `[N条] 名字` 当一行** → 必须 `_strip_count_prefix` 剥前导 `\s*\[?\s*\d{1,3}\s*条\s*\]?\s*`。
- **消息预览文字（`[草稿] 哈哈，这么!`）需要三重拒绝**：长度上限（预览>16字，真名2-12字）、句末标点（`。！!？?,，;；…：`）、预览关键词（`草稿/撤回/拍了拍/收到红包/添加了/邀请你`）。
- **`match_y_band` 默认 28**（不是 45）：行名到下方预览 ~25-35px，45 必跨行匹配到预览。
- **点击用 `name_y`（匹配到的名字中心Y）**，不是 `red_dot_y`（红点像素Y）—— 红点在头像右上角，名字在头像右中，垂直有 ~15px 偏差，用 `name_y` 才能精确点中行。
- 微信 4.x **窗口标题恒为"微信"**（联系人名在窗口内聊天头部），不能靠标题验证是否切换成功（曾考虑 post-click verify 靠标题判断，会误杀所有切换）。可靠 verify 需重抓侧栏看红点消失（重，~500ms/未读），默认不做。
