...
def _init_engine():
    ...
    with _ENGINE_LOCK:
        ...
        # ========= 回退PaddleOCR（V3回归修复：秒回，不在此同步加载） =========
        # 2026-08-25：a390279 曾把 "import paddleocr(实测~22s) + get_ocr()加载
        # + 自测推理" 塞进 _init_engine，导致首次识别/启动预热被卡 20~60 秒，
        # 表现"启动后卡死、零日志"。v3.4(c120a27) 的 _init_engine 秒回，模型
        # 加载延迟到 get_ocr()（由 main.initialize 的后台预热线程提前完成）。
        # 依赖健康诊断保留在 ui 诊断线程的 import 检查中。
        _ocr_engine = "paddle"
